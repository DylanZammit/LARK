...
class LARK(Kernels):
    ...
    def __init__(self, T, X, p, eps, kernel, drift=None, **kwargs):
        ...
        #self.birth = Birth(eps=eps, alpha=0.1, beta=1)

    def init_haar(self):
        J = poisson.rvs(10) # this should be a param
        self.J['haar'] = J
        self.W['haar'] = list(rand(J))
        self.B['haar'] = list(self.birth.rvs(size=J))
        self.s['haar'] = list(gamma.rvs(self.al, scale=1/self.bp, size=J))
    ...
    def init_expon(self):
        ...
        self.s['expon'] = list(gamma.rvs(self.al, scale=1/self.bl, size=J))
        self.p['expon'] = list(gamma.rvs(self.ap, scale=1/self.bp, size=J))

    def nu(self, t, p, s, W, B):
        out = self.b0
        if 'expon' in self.S:
            kernel = 'expon'
            out += sum([b*self.expon(t, w, p=p, s=s) for w, b, p, s in zip(W[kernel], B[kernel],
                                                                                           p[kernel], s[kernel])])
            #out += sum([b*self.expon_asym2(t, w, p=p[kernel], s=s[kernel]) for w, b in zip(W[kernel], B[kernel])])

        if 'haar' in self.S:
            kernel = 'haar'
            out += sum([b*self.haar(t, w, s=s) for w, b, s in zip(W[kernel], B[kernel], s[kernel])])
